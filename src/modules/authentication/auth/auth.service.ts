...
import { UnauthorizedError, BadRequestError, ForbiddenError } from "../../../helpers/error/app_error";
import { SESSION_EXPIRED, INVALID_LOGIN, INACTIVE_ACCOUNT } from "../../../helpers/error/error_response";
import { loggedInUserFields } from "../../user/user.dto";
import { UserStatus } from "../../user/user.enums";
import { IUser } from "../../user/user.model";
import { BIT, SessionDeactivationReason } from "../login_session/login_session.enum";
...
const loginUser = async (user: IUser, session?: ClientSession) => {
  ...
  if (activeSession) await logoutUser(activeSession, SessionDeactivationReason.USER_LOGIN, session);
  ...
  const loginSession = await loginSessionRepo.save(loginSessionData, { session });
...
const validateLoginSessionExpiry = async (loginSession: ILoginSession, session?: ClientSession) => {
...
const logoutUser = async (
  ...
  userSession = await loginSessionRepo.updateById(userSession.id, update, { session });

  return userSession;
};

const validateUserStatus = (user: IUser) => {
  if (user.status === UserStatus.DELETED) throw new BadRequestError(INVALID_LOGIN);
