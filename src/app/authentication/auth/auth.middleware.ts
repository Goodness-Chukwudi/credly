...
import { USER_LABEL, USER_PASSWORD_LABEL, LOGIN_SESSION_LABEL } from "../../../common/constants";
import { BadRequestError, NotFoundError, UnauthorizedError } from "../../../helpers/error/app_error";
import { EMAIL_REQUIRED, INVALID_LOGIN, generateNotFoundError, INVALID_AUTHENTICATION, SESSION_EXPIRED, INVALID_USER_SESSION } from "../../../helpers/error/error_response";
...
import { validateHashedData, getTokenFromRequest, verifyAccessToken } from "./auth.utils";
...
class AuthMiddleware extends RouterMiddleware {
   ...
   */
  loadUserAndPasswordToRequestByEmail = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!userPassword || !userPassword.user) throw new BadRequestError(INVALID_LOGIN);
   ...
   */
  loadUserToRequestByEmail = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const email = req.body.email;
      if (!email) throw new BadRequestError(EMAIL_REQUIRED);
  ...
  private verifyTokenCallback = (
    ...
    return async (
      ...
      try {
        ...
        if (!activeSession || !activeSession.user) throw new UnauthorizedError(INVALID_USER_SESSION);
